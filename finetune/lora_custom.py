"""Implementation derived from https://github.com/Lightning-AI/litgpt"""
...
def setup(
    data_dir: Path = Path("data/book"),
    checkpoint_dir: Path = Path("checkpoints/meta-llama/Llama-2-7b-chat-hf"),
    out_dir: str = "out/lora/custom_dataset",
    precision: Optional[str] = None,
    quantize: Optional[Literal["bnb.nf4", "bnb.nf4-dq", "bnb.fp4", "bnb.fp4-dq", "bnb.int8-training"]] = None,
) -> None:
    precision = precision or get_default_supported_precision(training=True)

    # edit out_dir to include model name, precision and quantization
...
def main(fabric: L.Fabric, data_dir: Path, checkpoint_dir: Path, out_dir: Path) -> None:
    ...
    model_name = "lit_model_lora_finetuned-lr" + str(learning_rate) + "-bs" + str(batch_size) + "-mbs" + str(micro_batch_size) + "-mi" + str(max_iters) + ".pth"
    save_path = out_dir / model_name
    save_lora_checkpoint(fabric, model, save_path)

    """ # save memory and time in json file
    resource_consumption = {}
    resource_consumption["model_name"] = model_name
    resource_consumption["learning_rate"] = learning_rate
    resource_consumption["batch_size"] = batch_size
    resource_consumption["micro_batch_size"] = micro_batch_size
    resource_consumption["gradient_accumulation_iters"] = gradient_accumulation_iters
    resource_consumption["max_iters"] = max_iters
    resource_consumption["memory"] = torch.cuda.max_memory_allocated() / 1e9
    resource_consumption["time"] = time.perf_counter()-train_time

    if os.stat("out/lora/custom_dataset/resource_consumption.json").st_size == 0:
        with open('out/lora/alpaca/resource_consumption.json', 'w') as outfile:
            outfile.write('[')
            json.dump(resource_consumption, outfile)
            outfile.write(']')
    else:
        with open('out/lora/custom_dataset/resource_consumption.json', 'rb+') as outfile:
            outfile.seek(-1, os.SEEK_END)
            outfile.truncate()
        with open('out/lora/custom_dataset/resource_consumption.json', 'a') as outfile:
            outfile.write(', \n')
            json.dump(resource_consumption, outfile)
            outfile.write(']') """
    ...
    for iter_num in range(1, max_iters + 1):
        ...
        if iter_num % log_interval == 0:
            ...
            fabric.print(
                ...
                f" {(t1 - iter_t0) * 1000:.2f}ms{' (optimizer.step)' if not is_accumulating else ''}", end="\r")

        if not is_accumulating and step_count % eval_interval == 0:
            t0 = time.perf_counter()
            val_loss = validate(fabric, model, val_data, tokenizer, max_iters=eval_iters)
            t1 = time.perf_counter() - t0
            fabric.print(f"step {iter_num}: val loss {val_loss.item():.4f}, val time: {t1 * 1000:.2f}ms", end="\r")
            fabric.barrier()
...
if __name__ == "__main__":
    ...
    micro_batch_sizes = [2]
    max_iters_ = [100, 500, 1000, 1500, 2000]

    # create hyperparameters configurations to run
    import itertools
    hparams_configs = itertools.product(learning_rates, batch_sizes, micro_batch_sizes, max_iters_)

    for [lr, bs, mbs, mi] in hparams_configs:
        learning_rate = lr
        batch_size = bs
